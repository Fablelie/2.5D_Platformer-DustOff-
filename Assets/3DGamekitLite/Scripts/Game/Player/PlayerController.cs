...
namespace Gamekit3D
{
    ...
    public class PlayerController : MonoBehaviour, IMessageReceiver
    {
        ...
        void SetTargetRotation()
        {
            ...
            if(moveInput.x != 0)
            {
                targetRotation = moveInput.x < 1 ? Quaternion.LookRotation(Vector3.left) : Quaternion.LookRotation(Vector3.right);
            } 
            else 
            {
            ...
            Vector3 resultingForward = moveInput.x < 1 ? Vector3.left : Vector3.right;//targetRotation * Vector3.forward;
